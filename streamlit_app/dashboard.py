...
if OPERATIONAL_SOURCE == "Supabase":
    ...
    st.caption(
        "Production-like dashboard consuming BigQuery marts generated by dbt "
        "from the configured demo operational data and orchestrated with Airflow."
    )
...
st.markdown("""
This dashboard covers the following HR analytics and operational workflow capabilities:

- Executive overview of jobs, applications, offers, hires, rejections and active applications.
- Application status distribution by office and department.
- Recruitment funnel analysis by stage.
- Recruiter workload monitoring.
- Recruiter SLA compliance on recruiter-owned stages.
- Recruiter SLA breach detection at application level.
- Average time to hire and time-to-hire diagnostics.
- Office and department scoped filtering.
- Candidate-level drilldowns where available.

The dashboard also includes a controlled Supabase admin workflow:

- Stage-change dry-run, approval and apply workflow.
- Hiring outcome dry-run, approval and apply workflow.
- Rejection outcome dry-run, approval and apply workflow.
- Candidate search and manual candidate/application selection.
- Business-readable candidate review before applying changes.
- Audit and technical details with request IDs and selected application IDs.
- Airflow analytics refresh trigger from Streamlit through the Airflow REST API.
- Airflow DAG run status monitoring when the API is reachable.
""")
...
```text
Generated mock recruitment dataset
    ↓
Mock Greenhouse API / API sync simulator
    ↓
...
Operational changes are not written directly to BigQuery.

Supabase is used as the demo operational source of truth for authentication, roles,
controlled mutations, dry-runs, approvals and auditability.

BigQuery remains the analytics warehouse. It is refreshed after controlled operational
changes through the Airflow/dbt pipeline.

Airflow orchestrates the end-to-end analytics refresh:
Supabase operational extraction, BigQuery ingestion, dbt transformations, dbt tests
and mart refresh.

The provided Excel and JSON files are used only to generate and seed the mock
recruitment dataset.
The dashboard does not read those files directly.

A maximum bytes billed limit is applied to BigQuery dashboard queries for cost control.
""")
